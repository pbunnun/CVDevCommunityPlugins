...
const QString Yolo8SegModel::_category = QString("Yolo Onnx");

const QString Yolo8SegModel::_model_name = QString( "Yolo8 Segmentation" );

Yolo8SegThread::Yolo8SegThread( QObject * parent, std::shared_ptr<BoundedThreadSafeQueue<cv::Mat> > pCVMatQueue)
    : QThread(parent)
{
...
readConfig( QString & config )
{
    try {
            ...
            QFile::exists( QString::fromStdString(labels_filename) ) )
        {
            if( mpSegmentator )
                delete mpSegmentator;
            mpSegmentator = new YOLOv8SegDetector(model_filename, labels_filename, bGPU);
...
late_constructor()
{
    if( !mpYolo8SegThread )
    {
        mpYolo8SegThread = new Yolo8SegThread(this, mpCVMatQueue);
        connect( mpYolo8SegThread, &Yolo8SegThread::result_ready, this, &Yolo8SegModel::received_result );
        load_model();
        mpYolo8SegThread->start();
    }
}
...
load_model()
{
    ...
    {
        mpYolo8SegThread->readConfig( msConfig_Filename );
    }
    else
    {
        QMessageBox err;
